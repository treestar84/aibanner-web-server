...
// 관리자 커뮤니티 글 편집 UI 전용 이미지 업로드. /api/v1/posts/upload-image와
// 거의 동일하지만 requirePostToken(기기 인증) 대신 requireAdminRequest(Basic
// Auth)로 게이트한다 — 관리자 화면에는 기기 post_token이 없다. 같은
// "community-posts/" prefix를 써서 사용자 업로드분과 한 폴더에서 관리된다.
export async function POST(req: NextRequest) {
  try {
    ...
    const blob = await put(
      `community-posts/${Date.now()}-${Math.random().toString(36).slice(2)}.${extension}`,
      file,
    ...
    console.error("[/api/admin/community-posts/upload-image][POST]", err);
